...
Requires `gridness_data.npz` beside the checkpoint (run `multi_seed_predictive_analysis.py` first).
```bash
python toroidal_structure_analysis.py \
  --checkpoint_path "Models/Single agent path integration/Seed 4 weight decay 1e-06/steps_20_batch_200_RNN_4096_relu_rf_012_DoG_True_periodic_False_lr_00001_weight_decay_1e-06/final_model.pth" \
  --n_batches 12 \
  --Ng_use 512 \
  --gridness_threshold 0.5 \
  ...
  --res 40
```
Outputs in `…/analysis_outputs/torus/` next to the checkpoint:
